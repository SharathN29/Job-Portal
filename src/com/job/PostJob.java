...
public class PostJob extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Connection con = DriverManager.getConnection(url, "root", "Typewriter");
			
			PreparedStatement ps = con.prepareStatement("insert into jobs(rid, role, compname, location, visa, comp, jd) values(?, ?, ?, ?, ?, ?, ?)");
			ps.setInt(1, rid);
			ps.setString(2, role);
